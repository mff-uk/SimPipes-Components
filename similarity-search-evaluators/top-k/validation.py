...
def main():
  ...
  evaluate_baseline(baseline, ids, distance_matrix, args['agg'], args['k'])
  return 0
...
def read_configuration():
  ...
  parser.add_argument("-k",
    type=int, dest="k", required=False, default=20,
    help="Determines top-k parameter.")
...
def evaluate_baseline(baseline, ids, distance_matrix, agg, k):
  sl, slcnt = 0, 0
  for i, case in enumerate(baseline):
    ...
    result = evaluate_baseline_case(case["outputs"], ids, similarity, k)
    if result is None:
    ...
    l = 0
    for idx in result:
      #logging.debug(f"{idx} ... position {result[idx]}")
      l += result[idx] <= k
    logging.info(f"{l/len(result)} accuracy")

    sl += l/len(result)
    slcnt += 1

  print(f"{(sl/slcnt)*100}% accuracy")
  #print(f"{(sl/k)*100}% precision\n{(sl/slcnt)*100}% recall")
...
def evaluate_baseline_case(outputs, ids, similarity, k):
  sort = dict(zip(
    map(
      lambda x: x, 
  ...
  for i in ids:
    if not ids[i] in sort:
      continue 
    if sort[ids[i]] <= k:
      logging.debug(f"[{i in outputs}] {i} ... {sort[ids[i]]}")
